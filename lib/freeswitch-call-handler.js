  ...
  return payloadParser(opts)
    ...
    .then((opts) => {
      ...
      if (rapidsosheaderscount > 0) {
		  ...
		  recording_organization = opts.recordingData['recording'].participant[0].extensiondata[0]['apkt:header'][3]['value'][0];
		  console.log("Forwarding call to Media Server for recording using filename: " + recording_filename + "-" + recording_organization);
              } else {
	...
	'X-RapidSOS-Record-Filename': recording_filename,
        'X-RapidSOS-Record-Organization': recording_organization
      };
